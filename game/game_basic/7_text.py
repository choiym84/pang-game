...
enemy_size = enemy.get_rect().size #이미지의 크기 얻어옴
enemy_width = enemy_size[0]
enemy_height = enemy_size[1]
enemy_x_pos = (screen_width / 2) - (enemy_width/2) #화면 중앙에 위치(가로)
enemy_y_pos = screen_height/2 - enemy_width #화면 맨 밑에 위치(세로)
...
while running:
    ...
    screen.blit(character,(character_x_pos,character_y_pos))#캐릭터 넣음
    
    screen.blit(enemy,(enemy_x_pos,enemy_y_pos))

    #타이머
    #경과 시간
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 #경과 시간을 1000으로 나누어 초단위로 표시한다

    timer = game_font.render(str(int(total_time - elapsed_time)),True,(255,255,255))
    # (시간, True, 글자 색상)이 들어간다
